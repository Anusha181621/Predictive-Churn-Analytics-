...
#: The callables the agent exposes, in the order they are offered to the model. Plain functions
#: with no decorator and no provider types, so every one can be called directly from a test with
#: no client, no key and no network -- and so swapping the model provider touches only the agent.
TOOL_FUNCTIONS = (
    book_summary,
    rank_customers,
    customer_detail,
...
def _schema(
    function: Any, description: str, properties: dict[str, Any], required: list[str] | None = None
) -> dict[str, Any]:
    """One tool declaration in the shape the chat-completions API expects."""
    return {
        "type": "function",
        "function": {
...
#: The declarations sent to the model. Written out rather than generated by introspection: these
#: descriptions are prompt text -- they are how the model decides which tool answers a question --
#: and prompt text deserves to be edited directly rather than assembled from docstrings.
#:
#: ``tests/test_assistant.py`` asserts that every function has exactly one schema and that no
#: schema names a parameter the function does not accept, so the two cannot drift apart unnoticed.
TOOL_SCHEMAS: list[dict[str, Any]] = [
    _schema(
        book_summary,
        "Summarise the whole customer base: how many customers there are, their average churn "
        "risk, how they split across risk bands, total revenue at risk and the campaign totals. "
        "Call this first for any question about the business as a whole, and to get your bearings "
        "before a more specific query.",
        {},
    ),
    _schema(
        rank_customers,
        "List the customers that rank highest on a measure, optionally narrowed to a group. The "
        "workhorse for 'who are the worst / most valuable / most at risk' questions.",
        {
            "order_by": {
                "type": "string",
                "enum": sorted(ORDERINGS),
                "description": "Measure to rank by, highest first.",
            },
            "limit": {
                "type": "integer",
                "description": f"How many customers to return, at most {MAX_ROWS}.",
            },
            "risk_level": {
                "type": "string",
                "description": "Optional. Keep only this risk band: Low, Medium, High or Critical.",
            },
            "segment": {
                "type": "string",
                "description": 'Optional. Keep only this behavioural segment, e.g. "Champions".',
            },
            "country": {"type": "string", "description": "Optional. Keep only this country."},
            "channel": {
                "type": "string",
                "description": "Optional. Keep only customers acquired through this channel.",
            },
            "category": {
                "type": "string",
                "description": "Optional. Keep only customers whose preferred category is this.",
            },
        },
    ),
    _schema(
        customer_detail,
        "Look up one customer: their churn risk, value, segments, recommended retention action "
        "and offer, and the ranked reasons the model gives for their risk.",
        {
            "customer_id": {
                "type": "string",
                "description": 'The customer identifier, e.g. "CUST0234".',
            }
        },
        ["customer_id"],
    ),
    _schema(
        aggregate,
        "Break a measure down by a dimension, to compare groups against each other — which "
        "segment, channel, country or city runs hottest.",
        {
            "dimension": {
                "type": "string",
                "enum": sorted(DIMENSIONS),
                "description": "What to group by.",
            },
            "measure": {
                "type": "string",
                "enum": sorted(MEASURES),
                "description": "What to measure per group.",
            },
        },
        ["dimension"],
    ),
    _schema(
        churn_drivers,
        "List what drives churn across the whole customer base, strongest first. For one "
        "customer's own reasons use customer_detail instead.",
        {
            "limit": {
                "type": "integer",
                "description": "How many drivers to return, at most 25.",
            }
        },
    ),
    _schema(
        model_summary,
        "Report how accurate the churn model is, with the caveats the training run recorded about "
        "itself. Use this for any question about whether the predictions can be trusted.",
        {},
    ),
]
...
def dispatch(name: str, arguments: dict[str, Any]) -> str:
    """Run the tool the model asked for, returning its result as a JSON string.

    An unknown name or a bad argument comes back as a tool result rather than an exception. The
    model can read that and correct itself; a raised exception would abandon the answer, and the
    model is the one thing here capable of recovering from its own mistake.
    """
    function = TOOLS_BY_NAME.get(name)
    if function is None:
        return _problem(f"There is no tool called {name!r}.", available=sorted(TOOLS_BY_NAME))
    try:
        return function(**arguments)
    except TypeError as error:
        return _problem(f"Wrong arguments for {name}: {error}")
    except Exception as error:  # noqa: BLE001 - a broken tool must not end the conversation
        return _problem(f"The {name} tool failed: {type(error).__name__}: {error}")
